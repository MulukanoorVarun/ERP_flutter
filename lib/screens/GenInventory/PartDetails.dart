...
class PartDetailsScreen extends StatefulWidget {
  ...
  final part_id;
  const PartDetailsScreen({Key? key, required this.part_id,}) : super(key: key);
  @override
...
class _PartDetailsScreenState extends State<PartDetailsScreen> {
  ...
  Future<void> LoadPartDetailsApifunction() async{
    ...
    try{
      await UserApi.LoadPartDetailsAPI(empId,session,widget.part_id).then((data)=>{
        if(data!=null){
          setState((){
            if(data.error==0){
  ...
  Future<void> UpdateIssueAPI() async{
    ...
    try{
      ...
      await UserApi.InventoryUpdateStockAPI(empId, session,issue_quantity.text,issue_description.text,widget.part_id,"Issued").then((data)=>{
        if(data!=null){
          setState((){
            if(data.error==0){
  ...
  Future<void> UpdateReceiveAPI() async{
    ...
    empId= await PreferenceService().getString("UserId")??"";
    try{
      await UserApi.InventoryUpdateStockAPI(empId, session,receive_quantity.text,receive_description,widget.part_id,"Received").then((data)=>{
        if(data!=null){
          setState((){
            if(data.error==0){
              toast(context, "Updated Successfully!");
            ...
            Column(
              children: [
                Container(
                  ...
                  child: Text(widget.part_id, style: GoogleFonts.ubuntu(
                    textStyle: TextStyle(
                ...
                Container(
                    child:Row(
                      mainAxisAlignment: MainAxisAlignment.start,
                      children: [
                        Text("Update issue", style: GoogleFonts.ubuntu(
                          textStyle: TextStyle(
                            fontSize: FontConstant.Size18,
                            fontWeight: FontWeight.w500,
                            overflow: TextOverflow.ellipsis,
              ...
              children: [
                ...
                Container(
                    child:Row(
                      mainAxisAlignment: MainAxisAlignment.start,
                      children: [
                        Text("Update Receive", style: GoogleFonts.ubuntu(
                          textStyle: TextStyle(
                            fontSize: FontConstant.Size18,
                            fontWeight: FontWeight.w500,
                            overflow: TextOverflow.ellipsis,
